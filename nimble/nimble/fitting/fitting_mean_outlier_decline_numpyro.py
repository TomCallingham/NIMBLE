# VERY FIRST CELL, before any JAX import
import os
...
def fit_numpyro_nuts_mean_outlier(
    ...
    # --- pull everything off device BEFORE releasing mcmc ---
    raw_samples = jax.device_get(mcmc.get_samples(group_by_chain=False))

    diagnostics = {}
    extra = mcmc.get_extra_fields()
    if extra:
        extra_host = jax.device_get(extra)
        if "diverging" in extra_host:
            diagnostics["num_divergences"] = int(np.sum(extra_host["diverging"]))
        if "num_steps" in extra_host:
            diagnostics["mean_tree_depth"] = float(
                np.mean(np.log2(extra_host["num_steps"] + 1))
            )

    # --- drop device state, clear JIT cache, force GC ---
    del mcmc, kernel
    jax.clear_caches()
    gc.collect()
    ...
    return raw_samples, diagnostics
...
def setup_and_fit_obs_mean_declining_outlier(
    ...
    **mcmc_kwargs,
    ...
    cleared so GPU memory is returned for the next fit.

    Returns
    -------
    sigma_samples, mean_samples, f_outlier_samples, multi_disp, diagnostics
        All arrays are numpy (host memory).
    """
    ...
    data_j = {
        ...
        # f_outlier is NOT in data_j — it is injected as a sampled JAX scalar
    ...
    raw_samples, diagnostics = fit_numpyro_nuts_mean_outlier(
        loglike_jax,
        nk=nk,
        data=data_j,
        vdisp_min=vdisp_min,
    ...
    # These are numpy arrays on host.
...
def compute_outlier_probabilities(
    ...
    The mean responsibility is transferred to host memory before returning,
    and the JAX JIT/compile cache is cleared so GPU memory is released for
    subsequent fits.
    ...
    # vmap over S samples, then average on device
    responsibilities = jax.vmap(responsibility_one_sample)(params_all, f_all)  # (S, N)
    p_out_device = responsibilities.mean(axis=0)  # (N,)

    # --- pull result to host, drop device arrays, clear JIT cache ---
    p_out_star = np.asarray(jax.device_get(p_out_device))

...
class DispersionMeanModel:
    ...
    def from_tuple_3d(cls, params_tuple, knots_logr):
        ...
        params_mean_theta = params_tuple[3 * num_knots : 4 * num_knots]
        params_mean_phi = params_tuple[4 * num_knots :]
        return cls(
            params_sigma_r,
            params_sigma_theta,
            params_sigma_phi,
    ...
    def mean_theta(self, log_r):
        return np.exp(agama.Spline(self.knots_logr, self.params_mean_theta)(log_r))

    def mean_phi(self, log_r):
        return np.exp(agama.Spline(self.knots_logr, self.params_mean_phi)(log_r))
...
class DispersionMeanMultiModel3D:  # For sampler
    ...
    def from_sampler_3d(cls, sigma_samples, mean_samples, knots_logr):
        ...
        params_mean_theta = mean_samples[:, :num_knots]
        params_mean_phi = mean_samples[:, num_knots:]
        return cls(
            params_sigma_r,
            params_sigma_theta,
            params_sigma_phi,
    ...
    def mean_theta(self, log_r):
        ...
        for i in range(self.n_sample):
            mean_theta[i, :] = agama.Spline(self.knots_logr, self.params_mean_theta[i, :])(log_r)
        return mean_theta
    ...
    def mean_phi(self, log_r):
        ...
        for i in range(self.n_sample):
            mean_phi[i, :] = agama.Spline(self.knots_logr, self.params_mean_phi[i, :])(log_r)
        return mean_phi
